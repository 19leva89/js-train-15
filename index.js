...
function calculateCircleArea(radius) {
	...
	if (typeof radius !== "number") {
		console.log("Помилка: значення радіуса не є числом")
		return null;
	}
...
function calculateVolumeCylinder(radius, height) {
	...
	if (typeof radius !== "number" || typeof height !== "number") {
		console.log("Помилка: радіус і висота не є числами")
		return null;
	}
	...
	// Округляємо об'єм до найменшого цілого числа числа що більше.
	const roundCylinderVolume = Math.ceil(cylinderVolume);

	// Повертаємо обчислений об'єм.
	return roundCylinderVolume;
}
...
function getFractionalPart(num) {
	...
	const fractionalPart = Math.fround(fractional);
...
function estimateInvestment(principal, interestRate, years) {
	...
	const summ = principal * Math.pow(1 + interestRate, years)

	// Округляємо ії до найближчого цілого
	const roundSumm = Math.round(summ);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
	...
	const totalPrice = products.reduce((accumulator, currentValue) => accumulator + currentValue.price, initialValue);

	// Конвертуємо totalPrice та maxPrice за допомогою Math.fround.
	const convertTotalPrice = Math.fround(totalPrice);
	const convertMaxPrice = Math.fround(maxPrice);
